module.exports = (expectedScopes, options) => {
  ...
  return (req, res, next) => {
    ...
    let subscopeKey = req.params.providerId || "1"
    
    if ( options &&
      options.customSubScopeKey != null &&
      typeof options.customSubScopeKey === 'string') {
        subscopeKey = options.customSubScopeKey
      }
    ...
    } else if (Array.isArray(req[userKey][scopeKey][subscopeKey])) {
      userScopes = req[userKey][scopeKey][subscopeKey];
    } else {
      return error(res);
    }
